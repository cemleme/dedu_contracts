...
contract DeduMarketplace is ReentrancyGuard, AccessControl, GroupApp, GroupStorage {
    using RLPDecode for *;
    using RLPEncode for *;
    ...
    address public constant CROSS_CHAIN =
        0xa5B2c9194131A4E0BFaCbF9E5D6722c873159cb7;
    address public constant GROUP_HUB =
        0x50B3BF0d95a8dbA57B58C82dFDB5ff6747Cc1a9E;
    address public constant GROUP_TOKEN =
        0x7fC61D6FCA8D6Ea811637bA58eaf6aB17d50c4d1;
    address public constant MEMBER_TOKEN =
        0x43bdF3d63e6318A2831FE1116cBA69afd0F05267;
    ...
    modifier onlyGroupOwner(uint256 groupId) {
        require(
            msg.sender == IERC721NonTransferable(GROUP_TOKEN).ownerOf(groupId),
            "MarketPlace: only group owner"
        );
        _;
    }
    ...
    function list(
        ...
        require(
            IGroupHub(GROUP_HUB).hasRole(
                ROLE_UPDATE,
                msg.sender,
                address(this)
            ),
            "Marketplace: no grant"
        );
    ...
    function delist(uint256 groupId) external onlyGroupOwner(groupId) {
        ...
        for (uint256 i; i < _salesRevenueRankingId.length; ++i) {
            if (_salesRevenueRankingId[i] == groupId) {
                for (
                    uint256 j = i;
                    j < _salesRevenueRankingId.length - 1;
                    ++j
                ) {
    ...
    function buy(uint256 groupId, address refundAddress) external payable {
        ...
        require(
            !_userPurchasedGroups[msg.sender].contains(groupId),
            "MarketPlace: already purchased"
        );
        require(
            msg.value >= prices[groupId] + _getTotalFee(),
            "MarketPlace: insufficient fund"
        );
    ...
    function buyBatch(
        ...
        for (uint256 i; i < groupIds.length; ++i) {
            ...
            require(
                !_userPurchasedGroups[msg.sender].contains(groupIds[i]),
                "MarketPlace: already purchased"
            );
        ...
        if (receivedValue > 0) {
            (bool success, ) = payable(refundAddress).call{
    ...
    function setFailureHandleStrategy(
        uint8 _failureHandleStrategy
    ) external onlyRole(OPERATOR_ROLE) {
    ...
    function _updateGroupCallback(
        ...
        (
            ...
            uint256 price,
            bool ok
        ) = _decodeCallbackData(_callbackData);
        if (!ok) {
